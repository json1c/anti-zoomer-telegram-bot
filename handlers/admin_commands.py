...
async def cmd_addword(message: types.Message):
    ...
    if len(args) == 0:
        ...
        db.add_local_word(message.chat.id, args[0:], message.from_user.id)
        await message.answer(
            get_template("commands:addword:success").format(word=message.get_args()[0:])
        )
...
async def cmd_delword(message: types.Message):
    ...
    if len(args) == 0:
        ...
        db.delete_local_word(message.chat.id, args[0:])
        await message.answer(
            get_template("commands:delword:success").format(word=message.get_args()[0:])
        )
...
async def cmd_addwhitelist(message: types.Message):
    ...
    if len(args) == 0:
        ...
        db.add_word_to_whitelist(message.chat.id, args[0:], message.from_user.id)
        await message.answer(
            get_template("commands:addwhitelist:success").format(
                word=message.get_args()[0:]
            )
        )
...
async def cmd_delwhitelist(message: types.Message):
    ...
    if len(args) == 0:
        ...
        db.delete_word_from_whitelist(message.chat.id, args[0:])
        await message.answer(
            get_template("commands:delwhitelist:success").format(
                word=message.get_args()[0:]
            )
        )
